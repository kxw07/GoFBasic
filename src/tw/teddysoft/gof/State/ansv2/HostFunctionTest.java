...
public class HostFunctionTest {
	...
	public void test_up_hard_state() {
		...
		host.powerOn();
		assertEquals("Cannot power on under up hard state.", 
				stream.toString().trim());
		stream.reset();
		
		host.powerOff();
		assertEquals("Power off gracefully.", 
				stream.toString().trim());
		stream.reset();
	...
	public void test_down_soft_state() {
		...
		host.powerOn();
		assertEquals("Cannot power on under down soft state.", 
				stream.toString().trim());
		stream.reset();
		
		host.powerOff();
		assertEquals("Cannot power off under down soft state.", 
				stream.toString().trim());
		stream.reset();
	...
	public void test_down_hard_state() {
		...
		host.powerOn();
		assertEquals("Power On.", 
				stream.toString().trim());
		stream.reset();
		
		host.powerOff();
		assertEquals("Cannot power off under down hard state.", 
				stream.toString().trim());
		stream.reset();
	...
	public void test_up_soft_state() {
		...
		host.powerOn();
		assertEquals("Cannot power on under up soft state.", 
				stream.toString().trim());
		stream.reset();
		
		host.powerOff();
		assertEquals("Power off after 60 second(s).", 
				stream.toString().trim());
		stream.reset();
		
		assertEquals("Diagnostic via the remote agent.", 
				host.diagnostic());
	}
}
